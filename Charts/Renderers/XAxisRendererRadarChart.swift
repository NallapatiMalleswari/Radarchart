...
open class XAxisRendererRadarChart: XAxisRenderer
{
    ...
    open override func renderAxisLabels(context: CGContext)
    {
        ...
        for i in stride(from: 0, to: chart.data?.maxEntryCountSet?.entryCount ?? 0, by: 1)
        {
            ...
            let p = ChartUtils.getPosition(center: center, dist: CGFloat(chart.yRange) * factor + xAxis.labelRotatedWidth / 4.0, angle: angle)
//            print("center \(center),chart.yRange \(chart.yRange), factor \(factor), \(xAxis.labelRotatedWidth), angle \(angle)")
            drawLabel(context: context,
                      formattedLabel: label,
                      x: p.x,
                      y: p.y - xAxis.labelRotatedHeight / 5.0,
